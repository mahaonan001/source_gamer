...
func SendMail(Toemail string, c *gin.Context) {
	...
	code := utils.RandomString(6, "1234567890")               // 验证码
	infTime := time.Now().Add(1 * time.Minute)
	//验证码可用
	if !IsEmailLegal(toUserEmail) {
		log.Println(toUserEmail)
		response.Response(c, http.StatusBadRequest, 402, gin.H{"msg": "err"}, "邮箱不合法")
		return
	}
	//验证码不可用
	...
	e.Subject = "欢迎使用____"                                                                                       // 邮件主题
	e.Text = []byte("验证码:" + code)                                                                               // 邮件正文内容（纯文本）
	e.HTML = []byte("<strong>" + string(e.Text) + "</strong><br><p>有效时长5分钟</p><p>如有问题请联系2022011536@cup.com</p>") // 邮件正文内容（HTML格式）
